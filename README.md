...
- Symptom: `--check` exits `0` but verified nothing useful.
- Likely cause: malformed manifest lines were skipped, or tagged manifest checked without `--tag`.
- Confirm: rerun with `--check --warn --strict` and inspect warnings.
- Fix: enforce `--strict`, and use `--tag` consistently for tagged manifests.

- Symptom: missing files still fail in `--check` mode even with `--ignore-missing`.
- Likely cause: `--ignore-missing` applies to hash mode, not verification failures.
- Confirm: run a manifest entry pointing to a missing file and observe `FAILED`.
- Fix: pre-filter manifest entries or treat missing files as explicit failures.

- Symptom: no output in CI despite verification running.
- Likely cause: `--quiet` or `--status`.
- Confirm: inspect command flags and process exit code.
- Fix: remove `--status`/`--quiet` when human-readable output is needed.

- Symptom: low throughput on many small files.
- Likely cause: job count too high for storage backend or metadata bottlenecks.
- Confirm: compare wall-clock times across `-j 1`, `-j 4`, `-j 8`, etc.
- Fix: tune `-j` to storage characteristics instead of CPU core count alone.

- Symptom: build/runtime consumers fail after upgrade (`libblake3.so.1` vs `.so.2`).
- Likely cause: ABI major changed.
- Confirm: check linked SONAME (`ldd`, package metadata).
- Fix: rebuild/relink consumers against `libblake3.so.2`.

## Limitations And Non-Goals

- CLI computes and verifies unkeyed 256-bit BLAKE3 only.
- CLI does not expose keyed or derive-key hashing; use the C library APIs for those modes.
- No recursive directory policy engine; you provide file paths.
- No built-in authenticity guarantees (signatures, trust store, key distribution).
- Check mode is line-oriented and only as strict as your flags (`--strict` matters).

## Compatibility & Stability

- Target platform: Linux (project and CLI paths are Linux-oriented).
- Architectures compiled in this tree: `x86_64` and ARM/`aarch64` variants via runtime dispatch and architecture-specific sources.
- Library ABI: current shared library major is `SONAME 2` (`libblake3.so.2`).
- API migration: legacy `b3p_hash_one_shot*` and `b3p_hash_buffer_serial` remain as raw-CV aliases; prefer safe wrappers (`b3p_hash_unkeyed*`, `b3p_hash_keyed*`, `b3p_hash_derive*`).
- Checksum parsing supports standard `hex  filename` and `hex *filename` lines.
- BSD-style `BLAKE3 ...` lines are supported when `--tag` is used consistently in generation and verification.
- Escaped filenames (`\\`, `\n`) are handled in text mode.

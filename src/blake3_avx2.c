/* src/blake3_avx2.c
 * AVX2 implementation of the compression function
 */
...
_Static_assert(sizeof(__m256i) == 32, "__m256i must be 32 bytes");

INLINE __m256i loadu(const uint8_t* src) {
    return _mm256_loadu_si256((const __m256i_u*)src);
}

INLINE void storeu(__m256i src, uint8_t* dest) {
    _mm256_storeu_si256((__m256i_u*)dest, src);
}
...
INLINE __m256i rot16(__m256i x) {
    static const __m256i mask = (const __m256i)_mm256_set_epi8(13, 12, 15, 14, 9, 8, 11, 10, 5, 4, 7, 6, 1, 0, 3, 2, 13, 12, 15, 14, 9, 8, 11, 10, 5, 4, 7, 6, 1, 0, 3, 2);
...
INLINE void load_counters(uint64_t counter, bool increment_counter, __m256i* out_lo, __m256i* out_hi) {
    ...
    __m256i carry = _mm256_cmpgt_epi32(_mm256_xor_si256(add1, _mm256_set1_epi32(0x80000000)), _mm256_xor_si256(l, _mm256_set1_epi32(0x80000000)));
...
blake3_hash8_avx2(const uint8_t* const* inputs, size_t blocks, const uint32_t key[8], uint64_t counter, bool increment_counter, uint8_t flags, uint8_t flags_start, uint8_t flags_end, uint8_t* out) {
    ...
    uint8_t block_flags = (uint8_t)(flags | flags_start);

    for (size_t block = 0; block < blocks; block++) {
        if (block + 1 == blocks) {
            block_flags = (uint8_t)(block_flags | flags_end);
        }

        const __m256i block_len_vec = set1(BLAKE3_BLOCK_LEN);
        const __m256i block_flags_vec = set1(block_flags);

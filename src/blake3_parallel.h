...
#define B3P_DEFAULT_SUBTREE_CHUNKS 4096

typedef enum {
  B3P_METHOD_AUTO = 0,
  B3P_METHOD_A_CHUNKS = 1,
  B3P_METHOD_B_SUBTREES = 2
} b3p_method_t;
...
BLAKE3_API int b3p_hash_one_shot(
  b3p_ctx_t *ctx,
  const uint8_t *input,
  size_t input_len,
...
BLAKE3_API int b3p_hash_one_shot_seek(
  b3p_ctx_t *ctx,
  const uint8_t *input,
  size_t input_len,
  const uint8_t key[BLAKE3_KEY_LEN],

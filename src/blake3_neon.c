...
INLINE uint32x4_t rot8_128(uint32x4_t x) {
  ...
  return vreinterpretq_u32_u8(__builtin_shufflevector(vreinterpretq_u8_u32(x), vreinterpretq_u8_u32(x), 1, 2, 3, 0, 5,
                                                      6, 7, 4, 9, 10, 11, 8, 13, 14, 15, 12));
...
void blake3_hash4_neon(const uint8_t* const* inputs,
                       size_t blocks,
...
INLINE void hash_one_neon(const uint8_t* input,
                          size_t blocks,
                          const uint32_t key[8],
                          uint64_t counter,
                          uint8_t flags,
